...
class AlocacaoController extends Controller
{
    ...
    public function add_lista($add_anima){
      $dados_lista  = Alocacao::all();
       return view("pages.alocacoes.lista", compact('dados_lista','add_anima'));
    }

    public function add_create($add_anima){
      return view("pages.alocacoes.create", compact('add_anima')); 
    }

    public function editar($id){
        $dados_editar = Alocacao::find($id);
        return view("pages.alocacoes.editar", compact('dados_editar'));  
    }
    ...
    public function store(Request $request){
        ...
        if($id_geral == ''){
          $tem = Alocacao::where('aldescricao', $request->input('aldescricao'))->get();
          ...
          if($dados->aldescricao !== $request->input('aldescricao')){
            $tem = Alocacao::where('aldescricao', $request->input('aldescricao'))->get();
            if(!count($tem) == 0){return 'erro, Já existe esse item cadastrado no sistema.';}
          }
        }
        $dados->aldescricao = $request->input('aldescricao');
